...
#define 	CS_LOW()	Chip_GPIO_SetPinState(LPC_GPIO, 0, 2, false);		/* Set CS low */
#define	CS_HIGH()	Chip_GPIO_SetPinState(LPC_GPIO, 0, 2, true);		/* Set CS high */
#define	IS_CS_LOW	!(Chip_GPIO_GetPinState(LPC_GPIO, 0, 2))		/* Test if CS is low */
#define	FORWARD(d)	uart0_putc(d)		/* Data forwarding function (console-out in this example) */
...
{
    uint32_t ticks_per_us = Chip_Clock_GetSystemClockRate() / 1000000;
    uint32_t ticks = ticks_per_us * us;

    Chip_TIMER_Init(LPC_TIMER32_0);
    Chip_TIMER_Reset(LPC_TIMER32_0);
    Chip_TIMER_SetMatch(LPC_TIMER32_0, 1, ticks);
    Chip_TIMER_ResetOnMatchEnable(LPC_TIMER32_0, 1);
    Chip_TIMER_Enable(LPC_TIMER32_0);

    while (Chip_TIMER_ReadCount(LPC_TIMER32_0) < ticks) {}
